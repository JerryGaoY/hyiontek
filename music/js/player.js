...
function listClick(no) {
    ...
    if(rem.dislist === 0) {
        ...
        // 查找当前的播放列表中是否已经存在这首歌
        for(var i=0; i<musiclist[1].item.length; i++) { if(musiclist[1].item[i].id="=" tmpmusic.id && musiclist[1].item[i].source="=" tmpmusic.source) tmpid="i;" playlist(tmpid); 找到了直接播放 return true; 退出函数 } 将点击的这项追加到正在播放的条目的下方 musiclist[1].item.splice(rem.playid + 1, 0, tmpmusic); 1; 正在播放 列表项已发生变更，进行保存 playersavedata('playing', musiclist[1].item); 保存正在播放列表 else 普通列表 与之前不是同一个列表了（在播放别的列表的歌曲）或者是首次播放 if((rem.dislist !="=" rem.playlist rem.dislist 1) || undefined) 记录正在播放的列表 musiclist[1].item="musicList[rem.playlist].item;" 更新正在播放列表中音乐 刷新正在播放的列表的动画 refreshsheet(); 更改正在播放的列表的显示 播放正在播放列表中的歌曲 参数：歌曲在列表中的id function playlist(id) 第一次播放 if(rem.playlist="==" pause(); 没有歌曲，跳出 if(musiclist[1].item.length <="0)" id 范围限定 if(id>= musicList[1].item.length) id = 0;
